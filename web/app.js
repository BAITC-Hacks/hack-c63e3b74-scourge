...
function fill(request){for(const key of ['city','category','format','date','budget','language','duration_hours']){const input=form.elements.namedItem(key);input.value=request[key]==null?'':String(request[key]);if(input.tagName==='SELECT')input.value=String(request[key]||'').toLocaleLowerCase('ru');}}
function render(data, day){
  ...
  if(!data.cards.length){
    const recovery=el('div',undefined,'empty');recovery.append(el('p',data.recovery_message));
    for(const suggestion of data.suggestions){
      const item=el('div',undefined,'suggestion');const button=el('button',suggestion.label);button.type='button';
      button.addEventListener('click',()=>{if(busy)return;fill(suggestion.request);compareMode(false);search();});
      item.append(button,el('p',suggestion.message,'hint'));recovery.append(item);
    }
...
function renderComparison(data){
  const section=el('section',undefined,'comparison-summary');section.append(el('h3','Что изменилось между датами'),el('p',data.message));
  if(data.changes.length){const list=el('ul');data.changes.forEach(change=>list.append(el('li',change.message)));section.append(list);}
  return section;
}
async function search(){
  ...
  try{
    const comparing=dates.length>1;
    const data=await api(comparing?'/api/compare':'/api/recommend',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(comparing?{...request,compare_date:dates[1]}:request)});
    if(comparing)results.append(renderComparison(data));
    (comparing?data.results:[data]).forEach(response=>results.append(render(response,response.request_date)));
    statusNode.textContent=comparing?'Одинаковые условия, две даты. Изменения объяснены по каждому профилю.':'Подбор готов. Порядок: меньшая цена, затем id.';
  }
