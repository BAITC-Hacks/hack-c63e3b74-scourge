...
FIELDS = {'city': nullable('string'), 'category': nullable('string'),
          'date': dict(nullable('string'), description=(
              'Дата YYYY-MM-DD только с явно указанным годом или однозначная относительная дата. '
              'Если год неизвестен или о нём задан вопрос, обязательно null; год today не подставлять.')),
          'budget': nullable('number'),
          'format': dict(nullable('string'), description=(
              'Тип мероприятия из текста с учётом падежа: «на свадьбу» → «свадьба», '
              '«для корпоратива» → «корпоратив». Обязательно заполни, если он явно назван; '
              'null только когда формат отсутствует или неоднозначен. Не путать с категорией подрядчика.')),
...
Перед ответом проверь каждое явно названное условие: его нельзя потерять при
выделении остальных полей или пожеланий. Формат события и категория подрядчика
независимы: «ведущий на свадьбу» задаёт category=ведущий И format=свадьба.
Примеры (остальные неупомянутые поля остаются null/[]):
«Нужен ведущий в Алматы» → city=алматы, category=ведущий, format=null, date=null.
«Ведущий на свадьбу 6 октября 2026» → category=ведущий, format=свадьба, date=2026-10-06.
«Фотограф на 14 ноября, год пока не определён» → category=фотограф, date=null,
questions=["Уточните год проведения мероприятия."] — даже если today содержит год.
Примеры иллюстрируют извлечение, не являются значениями по умолчанию.
'''
